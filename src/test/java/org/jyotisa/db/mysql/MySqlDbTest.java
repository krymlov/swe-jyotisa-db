...
public class MySqlDbTest extends AMySqlDbTest {
    public static final int DEFAULT_SECOND = i30;
    public static final int DEFAULT_MINUTE = i30;
    public static final int DEFAULT_HOUR = i12;
    public static final double TIME_ZONE = d0;

    @Test
    @Order(1)
    public void referenceKundali_2022_04_30() {
        final IKundali kundali = newKundali(newYearDateTime(2022, 4, 30));
        System.out.println(kundali);
    }

    @Test
    @Order(5)
    public void referenceKundali_2022_1_1() {
        final IKundali kundali = newKundali(newYearDateTime(2022, 1, 1));
        System.out.println(kundali);
    }

    @Order(10)
    @RepeatedTest(1)
    public void calculateKundali(RepetitionInfo rinfo) throws SQLException {
        calculateKundaliForYear(2022 - rinfo.getCurrentRepetition());
    }
    ...
    IKundali newKundali(final LocalDateTime ldt) {
        ...
        final SweJulianDate julianDate = new SweJulianDate(dt, (float)TIME_ZONE);
        ISweObjects sweObjects = new SweObjects(getSwephExp(), julianDate, GEO_LONDON, LAHIRI_CITRAPAKSA);
        return new Kundali(KUNDALI_8_KARAKAS, sweObjects.completeBuild());
    }
